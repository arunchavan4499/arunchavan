...
export default function BorderGlow({
    ...
    useEffect(() => {
        ...
        if (loop) {
            ...
            const tick = (now: number) => {
                const progress = ((now - t0) % cycleDuration) / cycleDuration;
                const pulse = 0.78 + 0.22 * Math.sin(progress * Math.PI * 2);
                setCursorAngle(angleStart + progress * 360);
                setEdgeProximity(pulse);
                rafId = requestAnimationFrame(tick);
            };
    ...
    const visibleEdgeProximity = alwaysOn ? Math.max(edgeProximity, 0.82) : edgeProximity;

    const borderOpacity = isVisible
        ? Math.max(0, (visibleEdgeProximity * 100 - colorSensitivity) / (100 - colorSensitivity))
        : 0;

    const glowOpacity = isVisible
        ? Math.max(0, (visibleEdgeProximity * 100 - edgeSensitivity) / (100 - edgeSensitivity))
        : 0;
